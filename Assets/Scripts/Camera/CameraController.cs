...
public class CameraController : MonoBehaviour
{
    ...
    public class CameraSettings
    {
        ...
        public float MaxClamAngle = 90;
        public float MinClamAngle = -30;
    ...
    {
        mainCam = Camera.main;
        UICam = mainCam.transform.GetComponentInChildren<Camera>();
    ...
    void RotateCamera()
    {
        ...
        cameraXrotation = Mathf.Clamp(cameraXrotation, cameraSettings.MinClamAngle, cameraSettings.MaxClamAngle);
    ...
    void ZoomCamera()
    {
        ...
        {
            mainCam.fieldOfView = Mathf.Lerp(mainCam.fieldOfView, cameraSettings.zoomFieldofView,
                cameraSettings.zoomSpeed * Time.deltaTime);
            UICam.fieldOfView = Mathf.Lerp(mainCam.fieldOfView, cameraSettings.zoomFieldofView,
                cameraSettings.zoomSpeed * Time.deltaTime);
        }
        else
        {
            mainCam.fieldOfView = Mathf.Lerp(mainCam.fieldOfView, cameraSettings.originalFieldofView,
                cameraSettings.zoomSpeed * Time.deltaTime); 
            UICam.fieldOfView = Mathf.Lerp(mainCam.fieldOfView, cameraSettings.originalFieldofView,
                cameraSettings.zoomSpeed * Time.deltaTime); 
        }
    }

    void HandlCamCollision()
    {
        if (!Application.isPlaying) return;
            ...
            out hit, cameraSettings.camCollisionLayers))
        {
            ...
            mainCam.transform.position = Vector3.Lerp(mainCam.transform.position, newCamPos, 
                Time.deltaTime * cameraSettings.moveSpeed);
        }
        else
        {
            mainCam.transform.localPosition = Vector3.Lerp(mainCam.transform.localPosition, InitialCamPos,
                Time.deltaTime * cameraSettings.moveSpeed);
        }
